...
function setDataStatistic() {
  connector.server.listClonedEsteira().done(function (jsonRecived) {

    let data = [];
    for (var x in jsonRecived.listModel) {
      data.push(jsonRecived.listModel[x]);
    }
    for (var x in jsonRecived.listArmazenamento) {
      data.push(jsonRecived.listArmazenamento[x]);
    }
    for (var x in jsonRecived.listEtiquetadora) {
      data.push(jsonRecived.listEtiquetadora[x]);
    }
    for (var x in jsonRecived.listDesvio) {
      data.push(jsonRecived.listDesvio[x]);
    }
    ...
    for (var x in data) {
      ...
      // ===== Graphics 1 load =====
      g1.data.labels.push(item.Name + " (" + item.Id + ")");
      var maior = null;
      var menor = null;
      var media = 0;
      ...
      // ===== Graphics 3 Load =====
      console.log(item);
      g3.data.labels.push(item.Name + " (" + item.Id + ")");
    ...
    if (totalFail > 0 || totalSuccess > 0) {
      let pct = 100/(totalSuccess + totalFail);
      g4.data.datasets[0].data[0] = Math.round((totalSuccess * pct)*100)/100;
      g4.data.datasets[0].data[1] = Math.round((totalFail * pct)*100)/100;
    }
